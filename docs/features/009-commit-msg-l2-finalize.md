...
- **canon-frozen-status set** — `{Approved, Implemented, Verified, Fix Applied, Current}`. Source: `cli/lint.py § CANON_FROZEN_STATUSES` (function-anchor; line numbers drift).
...
- [ ] **A6.** **`FINDING_REF_RE`** at `cli/lint.py` (top-level constant, near existing regex constants `CONVENTIONAL_PREFIX_RE` / `REFS_LINE_RE` / `METADATA_BLOCK_RE` — function-anchor; line numbers drift):
...
- [ ] D1. `docs/design/orchestra-philosophy.md` Changelog appended with v1.7 LLD-009 entry. **Verification: manual** (`grep "v1.7.0 shipped — commit-skill consolidation" docs/design/orchestra-philosophy.md`). Not lint-checkable; doc-deliverable.
- [ ] D2. `docs/runbooks/RUNBOOK-canon-inplace-violation-recovery.md` updated to reference tiered rule path (alongside existing supersession path). **Verification: manual** (`grep -E "tiered|BUG-011" docs/runbooks/RUNBOOK-canon-inplace-violation-recovery.md`). Not lint-checkable; doc-deliverable.
- [ ] D3. CONTRIBUTING.md mentions `cli.lint --pre-stage-check` for early-feedback workflow. **Verification: manual** (`grep "cli.lint --pre-stage-check" CONTRIBUTING.md`). Not lint-checkable; doc-deliverable.
...
- `cli/lint.py § CANON_FROZEN_STATUSES` — Glossary source (function-anchor; line numbers drift)
- `cli/lint.py § CONVENTIONAL_PREFIX_RE` / `REFS_LINE_RE` / `METADATA_BLOCK_RE` — existing regex constants; FINDING_REF_RE inserted alongside (function-name anchors; line numbers drift)
...
| 2026-05-11 | Addresses: docs/reviews/009-commit-msg-l2-finalize-r2.review.yaml gate completeness finding 1 (Minor) — added Verification classification (manual / lint-checkable / doc-deliverable) to D1-D3 Deliverables per r2 finding. |
| 2026-05-11 | Addresses: docs/reviews/009-commit-msg-l2-finalize-r2.review.yaml gate evidence finding 1 (Minor) — converted mixed line-anchored citations (cli/lint.py:71-73 + :128-130) to function-anchors (§ CANON_FROZEN_STATUSES, § CONVENTIONAL_PREFIX_RE) in Glossary + A6 + Related Documents. |
| 2026-05-11 | Addresses: docs/reviews/009-commit-msg-l2-finalize-r2.review.yaml gate consistency finding 1 (Minor) — verified A16 third-place CHANGELOG cite matches actual CHANGELOG.md v1.7.0 entry (150 → 259). Added post-ship actual numbers inline. |

...
- [ ] **A14.** **`ORCHESTRA_INIT_STRICT=1` env-var opt-in (per plan-r1 codex HIGH#3 + user interview-gate direction):** `cli.init` captures `cli.install_hooks.main(...)` return code. Default behavior (env unset): emit WARNING on non-zero rc; `cli.init` returns 0 unconditionally (non-blocking — preserves automation/CI workflows that expect `cli.init` success regardless of hook state). When `ORCHESTRA_INIT_STRICT=1` set: propagate non-zero rc from `cli.install_hooks` to `cli.init`'s exit code (fail-closed for CI/security-critical repos). Mirrors LLD-009 r3 ORCHESTRA_STRICT pattern. Tests T5e (default non-strict + non-zero rc → WARNING emitted + `cli.init` returns 0), T5f (`ORCHESTRA_INIT_STRICT=1` + non-zero rc → `cli.init` returns non-zero). Test count 15 → 16; pytest baseline 165 → 166.
...
| T7 | A9 | Full pytest baseline 150 + 16 new = 166 | baseline |

"""Stale-state + doc_disappeared tests for cli.spec_review (S23 + v1.6.1)."""
...
def test_doc_disappeared_between_dispatch_and_write(tmp_path, capsys, monkeypatch):
    """v1.6.1 finding #9 — A21 extension: doc deleted between dispatch + write → exit 1 + doc_disappeared."""
    from cli import spec_review
    ...
    def delete_then_dispatch(prompt):
        doc.unlink()
        return good

    monkeypatch.setattr(spec_review, "dispatch_subagent", delete_then_dispatch)

    rc = spec_review.main(["docs/features/008-foo.md"])
    assert rc == 1
    assert "doc_disappeared" in capsys.readouterr().err

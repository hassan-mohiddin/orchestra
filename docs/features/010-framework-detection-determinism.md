...
- [ ] **A4.** `cli.install_hooks --verify` entrypoint (NEW): post-install check. **Hybrid fingerprint per codex r2 HIGH#4 + user interview-gate direction:** substring floor (case-insensitive, full-file) AND entrypoint-pattern assertion. Asserts:
  - `.git/hooks/pre-commit` exists AND contains substring `orchestra` (raw mode) OR `generated by pre-commit` (framework mode) — **AND** contains expected entrypoint pattern: `python -m cli.lint --pre-commit` (raw) OR `pre-commit hook-impl` / `pre-commit run` (framework).
  - `.git/hooks/commit-msg` exists AND contains substring `orchestra` OR `generated by pre-commit` — **AND** contains expected entrypoint pattern: `python -m cli.lint --commit-msg-finalize` (raw) OR `pre-commit hook-impl` (framework).
  - Both stages present + both checks pass → exit 0 PASS.
  ...
  - **Hybrid rationale:** substring alone (r2) catches missing-hook + obvious-non-orchestra; entrypoint pattern catches stale/custom scripts that happen to mention `orchestra` in a comment but don't invoke the lint contract (codex r2 HIGH#4 false-PASS surface). Together: substring is the cheap robust floor; entrypoint pattern asserts execution semantics.
  - **Version floor:** pre-commit ≥3.0 supported. Older versions: documented as unsupported (Out of Scope).
  - Tests T4a (both present + raw fingerprint + raw entrypoint → PASS), T4b (both present + framework fingerprint + `pre-commit hook-impl` → PASS), T4c (commit-msg missing → FAIL with remediation), T4d (pre-commit missing → FAIL with remediation), T4e (file present but no fingerprint → FAIL), T4f (fingerprint at line 8+ but entrypoint present → PASS via widened scope), T4g (substring present BUT entrypoint pattern absent — stale orchestra-comment-only script → FAIL, codex r2 HIGH#4 false-PASS regression test).
- [ ] **A5.** `cli.install_hooks --apply` flag (NEW; opt-in auto-merge per Q1 grill answer): when `.pre-commit-config.yaml` present + `--apply` passed: parse user config (parse error → A3 hard-fail); deep-merge orchestra repos entry (append-as-new-local-block per Q4); preserve user's other repos entries; preserve all existing top-level keys; ensure `default_install_hook_types` covers both `pre-commit` AND `commit-msg`. **Transactional rollback (per codex r2 HIGH#3):** before any write, snapshot original config bytes to `.pre-commit-config.yaml.orchestra-backup` (sibling file, not in `.git`). Sequence: snapshot → atomic-write modified config → run `pre-commit install` subprocess → run `--verify`. On ANY failure (subprocess non-zero rc OR verify FAIL): restore from backup via `os.replace(backup, cfg_path)`; emit explicit error including rollback notice; exit non-zero. On success: leave backup file in place for 1 successful re-run window then delete (or unlink immediately — defer to impl). Closes the partial-state-after-install-fail surface where comments-stripped config persisted without orchestra hooks. **Canonical durable-write pattern** (per r1 orchestra evidence #3 + sonnet F2):
...
- [ ] **A11.** Pytest target: LLD-009 r5 baseline (**218**, updated through r3→r4→r5 cascade per Plan r3) + LLD-010 new tests (**27 distinct test functions**) → **≥245** (LLD-010-owned tests). Enumeration: T1=1; T2=1; T3a-d=4; T4a-g=7 (added T4g r2 hybrid stale-script regression); T5a-h=8 (added T5g+T5h r2 transactional rollback); T6a-d=4; T7a-b=2; T8=1; T-INT-010=1 → 27. **Combined v1.7.0 ship target = 248** per Plan r3 (LLD-010 245 + Plan Phase 4 migration delta). r4 absorbs cascade only; no LLD-010-internal test additions.
- [ ] **A12.** CHANGELOG.md v1.7.0 entry (LLD-010 portion) describes: framework-detection redesign, snippet correctness (**pass_filenames: false on both hook ids**; git's native commit-msg contract injects msg-file as `$1` independently — corrects r2 stale "pass_filenames: true" wording per codex r2 HIGH#1 + orchestra r2 clarity finding), `--apply` opt-in auto-merge with transactional rollback, `--verify` entrypoint with hybrid fingerprint check (substring + entrypoint pattern), BUG-006 close.
...
2. **Framework-detection redesign** — parse user config + emit full snippet template verbatim. (Tailoring removed in r2 per r1 sonnet F3; codex r2 HIGH#2 caught residual "tailored" wording here in r2 In-Scope text — r3 corrects.)
...
### `--apply` transactional rollback (per A5 + codex r2 HIGH#3; r3)

```python
def install_via_framework_apply_with_rollback(repo_root: Path) -> int:
    """--apply with transactional rollback: snapshot config; on any subsequent
    failure (subprocess or verify), restore original config bytes."""
    cfg_path = repo_root / PRECOMMIT_CONFIG
    backup_path = cfg_path.with_name(cfg_path.name + ".orchestra-backup")

    # 1. Snapshot original config bytes BEFORE any modification
    if cfg_path.exists():
        backup_path.write_bytes(cfg_path.read_bytes())
    else:
        # New install — no backup needed; "rollback" = delete the file we created
        backup_path.touch()
        backup_path.write_text("__ORCHESTRA_NEW_INSTALL__")

    def _rollback(reason: str) -> int:
        if backup_path.read_text(errors="replace") == "__ORCHESTRA_NEW_INSTALL__":
            cfg_path.unlink(missing_ok=True)
        else:
            os.replace(backup_path, cfg_path)
        backup_path.unlink(missing_ok=True)
        print(f"error: {reason}. Original .pre-commit-config.yaml restored from backup.", file=sys.stderr)
        return 1

    # 2. Run install_via_framework_apply (modifies config + runs pre-commit install + verify)
    try:
...
graph TD
    ...
    D -->|yes| C
    D -->|no| E["parse user config<br/>(parse error → hard-fail)"]
    E --> F{"orchestra ids state?<br/>(none | partial | full)"}
    F -->|full| G["exit 0: 'already configured' + verify"]
    F -->|partial| P["exit non-zero:<br/>A3b XOR hard-fail<br/>(manual merge or --force-raw)"]
    F -->|none| H{"--apply flag?"}
    H -->|no| I["print snippet + instructions; exit 0"]
    H -->|yes| BK["snapshot config to<br/>.orchestra-backup (A5 rollback)"]
    BK --> J["auto-merge: append local repo entry"]
    J --> K["write user config atomically<br/>(with-block + flush + fsync + dirfsync)"]
    K --> L["subprocess: pre-commit install both types"]
    L -->|rc != 0| RB["rollback config<br/>from backup; exit non-zero"]
    L -->|rc == 0| M["cli.install_hooks --verify<br/>(substring + entrypoint pattern)"]
    M -->|both stages + entrypoints present| N["exit 0: PASS + delete backup"]
    M -->|missing| RB
...
```python
RAW_FINGERPRINT = "orchestra"  # case-insensitive substring floor
FRAMEWORK_FINGERPRINT = "generated by pre-commit"  # case-insensitive substring floor

# Per codex r2 HIGH#4 + user interview-gate hybrid: entrypoint pattern per stage
EXPECTED_ENTRYPOINTS = {
    "pre-commit": [
        "python -m cli.lint --pre-commit",       # raw mode
        "pre-commit hook-impl",                    # framework mode
        "pre-commit run",                          # framework legacy
    ],
    "commit-msg": [
        "python -m cli.lint --commit-msg-finalize",  # raw mode
        "pre-commit hook-impl",                       # framework mode
    ],
}
...
def verify_hooks_active(repo_root: Path) -> int:
    """Verify both pre-commit and commit-msg .git/hooks/<name> exist + contain
    BOTH fingerprint floor AND expected entrypoint pattern.

    Hybrid check (codex r2 HIGH#4): substring catches missing-hook;
    entrypoint-pattern catches stale-script false-PASS where file contains
    'orchestra' comment but doesn't actually invoke the lint contract.
    """
    findings: list[str] = []
    for hook_name in ("pre-commit", "commit-msg"):
        ...
        if RAW_FINGERPRINT.lower() not in content_lower and FRAMEWORK_FINGERPRINT.lower() not in content_lower:
            ...
            continue
        # Hybrid: entrypoint pattern (case-sensitive — entrypoint string is exact)
        expected_any = EXPECTED_ENTRYPOINTS[hook_name]
        if not any(pattern in content for pattern in expected_any):
            findings.append(
                f"{hook_name}: fingerprint present but no expected entrypoint pattern found. "
                f"Expected one of: {expected_any}. "
                f"Likely a stale or custom hook script with orchestra comment but no lint invocation."
            )
    if findings:
...
| T4g | A4 | Hybrid: substring `orchestra` comment present BUT no entrypoint pattern (`cli.lint --pre-commit` / `hook-impl`) → verify FAIL (stale-script false-PASS regression per codex r2 HIGH#4) |
...
| T5g | A5 | Transactional rollback: mock pre-commit install rc=1 → original config restored byte-for-byte from .orchestra-backup; error msg mentions rollback (codex r2 HIGH#3) |
| T5h | A5 | Transactional rollback: verify-fail post-install → original config restored from backup |
...
Total: **27 distinct test functions** (added T4g + T5g + T5h in r3).

Pytest target post-LLD-010-ship: 213 (LLD-009 r3 baseline) + 27 (LLD-010) = **≥240** (combined v1.7.0 target).
...
- `docs/reviews/010-framework-detection-determinism-r2.review.yaml` — orchestra judge-1 r2 attestation (conditional_pass; 2 Minor)
- `docs/reviews/010-framework-detection-determinism-r2.codex.md` — codex judge-2 r2 review (4 HIGH; drove r3 incl. transactional rollback + hybrid verify)
...
| 2026-05-11 | Status: Draft → Implemented. Phase 3 shipped at commit `da7238d` (30 new tests; baseline 258; T5d xfail flipped to pass). BUG-006 closed (Status flip in Phase 5 batch). |
| 2026-05-11 | r3 → r4 paperwork-only cascading edit per plan-r2 orchestra Evidence Minor #1 + user-delegated decision: A11 pytest baseline updated to absorb LLD-008 r6→r7 cascade (166 → 167) + LLD-009 r4→r5 cascade (216 → 218) + Plan r3 Phase 4 migration tests (3 new beyond r2's 5). LLD-010 r4 adds no internal test changes. Combined v1.7.0 ship target: 243 → 248. r4 captured by plan r3 spec-review per user direction. Status: Draft. |
| 2026-05-11 | r2 spec-review verdicts: orchestra conditional_pass (2 Minor); codex needs-attention (4 HIGH — fresh architectural surface, partial overlap with orchestra Minor on A12 stale wording). r2 → r3 fixes inline per user interview-gate direction: A12 CHANGELOG description corrected to `pass_filenames: false on both hook ids` (codex r2 HIGH#1 + orchestra Minor #2); In-Scope item 1 "tailor snippet" wording removed → "emit full snippet template verbatim" (codex r2 HIGH#2 internal contradiction with A3); A5 `--apply` transactional rollback added (codex r2 HIGH#3) — snapshot config to `.orchestra-backup` before write; restore on subprocess/verify failure; new `install_via_framework_apply_with_rollback` function + T5g + T5h tests; A4 `--verify` strengthened to hybrid (substring floor + entrypoint pattern assertion `python -m cli.lint --pre-commit` raw / `pre-commit hook-impl` framework per codex r2 HIGH#4 + user interview-gate direction) — closes stale-script false-PASS; new T4g regression test; Mermaid diagram updated to show A3b XOR partial-config branch (orchestra Minor #1) + rollback paths + verify hybrid label. Test count 24 → 27 (T4g + T5g + T5h); pytest combined target 230 → 240 (LLD-009 r3 baseline 213 + 27 = 240). Status: Draft. r3 final iteration before impl per user direction. |

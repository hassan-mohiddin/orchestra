...
> **Status:** Implemented
> **Iteration:** 8
> **Supersedes:** docs/archive/features/008-commit-skill.md
...
- [ ] **A3.** Hook templates relocated `cli/templates/{pre-commit.sh,commit-msg.sh}` → `skills/commit/templates/`. **r8 update:** `attestation-template.yaml` relocated `cli/templates/` → `skills/spec-review/templates/` per architectural cleanup (spec-review skill owns its own templates per LLD-008 placement rule). **Post-BUG-016 slice 8 sync (2026-05-12):** `cli/templates/` now retains **11 init-related artifacts** (was 10 post-r8-supersession, was 11 in r7): mkdocs.yml, AGENTS.md.template, llms.txt.template, standards-default-7.md, docs-index.md, mkdocs_hooks.py, orchestra-lint.yml, requirements-docs.txt, tags.md, precommit-yaml-patch.txt, vocabulary-default-1.md. `precommit-yaml-patch.txt` stays — different purpose (BUG-007 yaml-checker `--unsafe`). `vocabulary-default-1.md` added by BUG-016 slice 8 (commit `937a5f0`) — generated from `docs/design/controlled-vocabulary.md` canon §4.x via `scripts/generate_vocab_template.py`. → test T2 (presence assertions on cli/templates/ + skills/commit/templates/ + skills/spec-review/templates/; full-enumeration variant `test_cli_templates_dir_full_enumeration` at `tests/test_install_hooks_skill_dir.py` shipped post-BUG-012 r5 closure).
...
- [ ] **A8.** SCALE-side migration commit (separate ship; bundled with orchestra v1.7.0):
  ...
  - REWRITE-IN-PLACE `SCALE/.claude/rules/documentation-gate.md`:
    ...
    - in the `## Quick Reference` section (located by `## Quick Reference` markdown header — line ranges drift with prior edits) of `SCALE/.claude/rules/documentation-gate.md`, retain Gates 1-3 bullets verbatim AND replace the Gate 4 + Gate 5 bullets with one-line pointer: `Gate 4 / Gate 5 → orchestra:commit skill (see references/canon-frozen-guard.md + references/commit-strategy.md)`. Net: 4 bullets retained, 2 collapsed to 1 pointer.
...
```python
def install_one_hook(repo_root, hook_name, on_conflict=None, force=False, input_fn=input):
    # input_fn=input is the test-injection seam (tests pass lambda for prompt mock)
    # ... existing setup
...
- **Field name `Skill-Status` (NOT `Status`)** — intentionally distinct from doc-lifecycle `Status` enum used by `cli.lint`. This prevents future-maintainer confusion if `skills/` is ever added to `REFS_ELIGIBLE_PREFIXES`: the skill-internal lifecycle would not collide with the canon-frozen-status set `{Approved, Implemented, Verified, Fix Applied, Current}` because the field name differs. **Value-collision example:** even though the skill `Skill-Status: Current` token-overlaps with the doc-lifecycle `Status: Current` value, a value-based scanner that ignores field name would conflate the two; intentional field-name divergence (`Skill-Status` vs `Status`) blocks this collision. Documented as intentional vocabulary split.
...
| 2026-05-11 | Addresses: docs/reviews/008-commit-skill-r4.review.yaml gate clarity finding 1 (Minor) — added inline `input_fn=input` comment in install_one_hook pseudocode marking the test-injection seam (Design § cli.install_hooks non-interactive bootstrap). |
| 2026-05-11 | Addresses: docs/reviews/008-commit-skill-r4.review.yaml gate consistency finding 2 (Minor) — replaced fragile `lines 109-128` SCALE documentation-gate.md citation in A8 with section-header anchor (`## Quick Reference`) to survive future edits. |
| 2026-05-11 | Addresses: docs/reviews/008-commit-skill-r4.review.yaml gate clarity finding 2 (Minor) — added inline note to A5 documenting that the existing interactive prompt path is preserved verbatim from v1.5+ (two stderr `print()` calls + `input_fn("> ")` separator) so contributor muscle memory unaffected. |
| 2026-05-11 | Addresses: docs/reviews/008-commit-skill-r4.review.yaml gate consistency finding 3 (Minor) — added explicit value-collision example to Skill-Status field-name divergence note (e.g., Skill-Status: Current token-overlaps with doc-lifecycle Status: Current; field-name divergence blocks scanner conflation). |
| 2026-05-11 | r7 → r8 supersession (canon-frozen contract A3 enumeration drift). attestation-template.yaml moved `cli/templates/` → `skills/spec-review/templates/` per architectural cleanup. A3 list adjusted: 11 → 10 init-related artifacts in cli/templates/; new skills/spec-review/templates/ directory introduced. Closes BUG-012 §Post-ship cleanup observation #5. Status: Implemented (r8 carries forward r7 ship state; supersession is contract-correctness, no implementation change). |
| 2026-05-12 | Addresses: docs/reviews/BUG-012-v17-1-minor-followups-r5.orchestra.review.yaml gate adversarial finding 4 (Important) — A3 enumeration synced 10 → 11. BUG-016 slice 8 (commit `937a5f0`) added `cli/templates/vocabulary-default-1.md` (generated from canon `docs/design/controlled-vocabulary.md` §4.x via `scripts/generate_vocab_template.py`) without syncing A3 prose. Test T2 full-enumeration variant (`tests/test_install_hooks_skill_dir.py::test_cli_templates_dir_full_enumeration`) was already asserting 11 files; A3 prose is now in lockstep. Tiered narrow-change per LLD-009 r6 §A6 (v2.0 attestation gate=adversarial finding=4 Important, validator support shipped via BUG-018 v2.0.1 fix). |

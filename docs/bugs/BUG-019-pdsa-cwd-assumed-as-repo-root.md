...
- r2-fix (2026-05-12) — fix shipped at commit `81f8e59`. `cli/pdsa.py` adds `_discover_repo_root(anchor)` (functools.lru_cache, sanitized env clearing GIT_DIR/GIT_WORK_TREE/GIT_CEILING_DIRECTORIES, fallback None on non-git-repo). `_resolve_inside_repo` + `_check_class_audit_attestation` rewritten to use the discovered root; `_check_class_audit_attestation` fail-closed when discovery returns None (consistent with `_resolve_inside_repo`). Test surface: 35 tests in `tests/test_pdsa.py` + 8 in `tests/test_class_vs_instance.py` updated with `monkeypatch.setattr(pdsa, "_discover_repo_root", lambda anchor: tmp_path)` since tmp_path fixtures are not real git repos. New `tests/test_pdsa_repo_root_discovery.py` ships 10 regression cases including env-var sanitization (test_10) and submodule layout (test_7) using real `git init` fixtures. Acceptance: pytest 610 pass, pyrefly 0 errors, `cli.lint --pre-commit` clean, real-bug dogfood from `docs/plans/` CWD reports `_check_citations.passed=True`. Status: Investigating → Fix Applied.
...
| 2026-05-12 | Fix shipped at `81f8e59`. `cli/pdsa.py::_discover_repo_root` lru-cached helper invokes `git rev-parse --show-toplevel` with sanitized env (clears GIT_DIR/GIT_WORK_TREE/GIT_CEILING_DIRECTORIES). `_resolve_inside_repo` + `_check_class_audit_attestation` rewired; both fail-closed when discovery returns None. Existing 35 tests in `test_pdsa.py` + 8 in `test_class_vs_instance.py` got `monkeypatch.setattr(pdsa, "_discover_repo_root", lambda anchor: tmp_path)` for tmp_path fixtures. New `test_pdsa_repo_root_discovery.py` ships 10-case regression including env-var sanitization (test_10) + submodule layout (test_7) using real `git init`. Acceptance: pytest 610 pass, pyrefly 0 errors, `cli.lint --pre-commit` clean, real-bug dogfood from `docs/plans/` CWD reports `_check_citations.passed=True`. Closes GH issue #9. Status: Investigating → Fix Applied. |

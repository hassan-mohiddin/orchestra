...
> **Status:** Current
> **Iteration:** 2
> **Supersedes:** docs/archive/design/orchestra-philosophy.md
...
gantt
    ...
    v1.1 — design-docs init sub-skill       :active, v11, 2026-05, 14d
...
| ADR-002 | Module layout: flat `cli/<module>` (no `orchestra/` Python package) | Captured in LLD-001 § Structure (no separate ADR filed) | Implemented |
| ADR-003 | Mermaid lint reuses existing `extract_mermaid.py` (no duplication) | Captured in LLD-001 § Mermaid (no separate ADR filed) | Implemented |
| ADR-004 | Plugin manifest spec (`orchestra.json` for plugin authors) | Deferred to v1.5+ workflow skill (LLD-011) | Deferred |
| ADR-005 | RFC vocabulary stance — ADR-only across both modes | Captured in this Design Doc § Formal Vocabulary (no separate ADR filed) | Implemented |
...
| 2026-05-10 | v1.5.0 shipped (LLD-006-r4 → Implemented). 107 pytest + 11/11 evals. Plugin version bumped 1.3.0 → 1.5.0 (1.4 burnt; rolled back 2026-05-07, never released). New CLI: cli.lifecycle reject + update-attestation-paths; cli.lint --attestations. New lint checks L1-L4: Refs:-eligibility / canon-frozen narrow-change / attestation path-mutation / doc-id-burn. Conventions: `docs/archive/<type>/` for Rejected+Superseded, supersession workflow with `-rN` revisions, doc-id-burn policy. Dogfood migration: 5 archived rejected docs (LLD-005 r1+r2, LLD-006 r1+r2+r3). Deps: python-frontmatter>=1.1, PyYAML>=6.0. Status remains Current. |
...
| 2026-05-12 | LLD-012 SC-13: orchestra TLDR-compression + hook-based re-injection lineage cited explicitly. Karpathy K1/K2/K3/K4/K8 ("Raw / Wiki / Schema layer" file taxonomy; log.md as immutable evidence; lint-as-ritual quality gate; schema-as-config; wiki compiles knowledge). The 3-layer file taxonomy (raw chat → wiki summary → schema rules) drove orchestra's TLDR-on-schema-layer design, where `## TLDR — Nonnegotiables` sections in `.claude/CLAUDE.md` + `.claude/rules/*.md` are the schema layer; lessons in `docs/lessons/<YYYY-MM>-lessons.md` are the raw layer; spec-review attestations are the wiki/log layer. Status: Current (whitelist Changelog-only edit; body unchanged). |

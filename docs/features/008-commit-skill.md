...
- [ ] **A5.** `cli.install_hooks` adds `--on-conflict={skip,replace,append}` flag (default `skip`). Non-interactive when flag passed. Existing interactive prompt path retained when flag absent + `sys.stdin.isatty()` returns True; replaced with `skip` default when non-TTY (CI / scripts) → tests T4a (skip leaves untouched), T4b (replace overwrites), T4c (append concatenates), T4d (non-TTY without flag defaults to skip).
- [ ] **A6.** `cli.init` invokes `python -m cli.install_hooks --pre-commit --commit-msg --on-conflict=skip` post-init operations (BUG-010 Part 3). Idempotent. Non-blocking in CI. Tests T5a (cli.init bootstrap leaves both hooks installed), T5b (idempotent re-run), T5c (cli.init + non-TTY = no input() blocking).
...
- [ ] **A8.** SCALE-side migration commit (separate ship; bundled with orchestra v1.7.0):
  ...
  Counts: 2 deletions + 1 partial-edit + 1 registry-append. Test T6 (post-migration SCALE state assertions; runs in SCALE repo).
...
- [ ] **A12.** BUG-010 Part 3 (auto-install bootstrap) closes via A6 implementation. BUG-010 stays `Fix Applied`; Part 3 was tracked as open sub-task per BUG-010 Iteration Log r1.
...
2. **references/ canonicalization** — 5 reference files with content sourced from SCALE-side `.claude/rules/{canon-frozen-guard,commit-strategy}.md` plus 3 NEW reference files extracted from existing prose (refs-line-rules, doc-vs-code-commit, supersession-decision).
...
- **5 deferred BUGs (BUG-001/002/004/005)** — separate v1.7.x ships.
...
def install_one_hook(repo_root, hook_name, on_conflict="skip", force=False, ...):
    ...
    if hook_path.exists() and existing != expected:
        # Resolve conflict per flag (or TTY fallback)
...
- Each reference file has metadata block: `Doc ID`, `Date`, `Status` (skill-internal enum: `Current` / `Deprecated`), `Skill version`.
...
### Phase impl — new test matrix (7 IDs; 11 distinct functions)
...
| T3 | A4 | install_hooks reads from skills/commit/templates/ (path resolution + install + content) | yes |
...
| T4d | A5 | Non-TTY without flag defaults to skip (no input() blocking) | yes |
| T5a | A6 | cli.init bootstrap leaves both hooks installed | yes |
| T5b | A6 | cli.init bootstrap idempotent re-run | yes |
| T5c | A6 | cli.init + non-TTY = no input() blocking | yes |
| T6 | A8 | SCALE migration: 2 absent files, doc-gate.md retains Gates 1-3 sections + skill pointer for Gates 4+5, registry entry present | yes |
| T7 | A9 | Full pytest baseline 150 + 11 new = 161 | baseline |

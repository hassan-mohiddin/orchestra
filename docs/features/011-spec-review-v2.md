...
| Stricter YAML canonicalization (RFC-spec YAML 1.2 emitter, not PyYAML safe_dump) for `attestation_integrity_hash` | Deferred to v2.1+ | Per iter-2 adversarial finding 1: PyYAML safe_dump is deterministic within a 6.x patch-version family but cross-version verification is not guaranteed. v2.0 pins the PyYAML 6.x family and treats this as the algorithm-of-record. v2.1 may swap to an explicit YAML 1.2 canonicalizer if cross-implementation verification becomes a real-world requirement. Tracked as a class-known limitation. |
| Aggregator second-canonical-text preservation on fuzzy_hash collision | Deferred to v2.1+ | Per iter-2 adversarial finding 2: when distinct findings collide on (location, fuzzy_hash), aggregator picks alphabetically-first sub-judge text and the other distinct framing is dropped. raised_by length>1 surfaces the collision; full preservation (e.g., a `merged_from` list of alternate problem strings) is a v2.1 enhancement that adds schema surface. Tracked as a class-known limitation. |
| Mandatory-tier vendor diversification (semantic on Opus, adversarial on Sonnet, etc.) | Deferred to v2.1+ | Per iter-2 adversarial finding 4: both mandatory sub-judges run on `claude-opus-4-7` → any Opus outage forces overall_verdict=fail on every review. Operator recovery via `--override-cap` (degraded mode) is documented. v2.1 will introduce a sub-judge model-config layer so the mandatory tier can be redistributed across vendors. Tracked as a class-known SPOF; documented in §E17b. |
...
- `overall_verdict`
- `overall_verdict_basis`
- `attestation_integrity_hash` — SHA-256 over canonical YAML payload with this field zeroed out; computed at write time, re-verified at iter-2 load. **Canonicalization algorithm-of-record** (slice 2.4 of LLD iter-2): the canonical form is `yaml.safe_dump(payload_with_hash_field_removed, sort_keys=True, default_flow_style=False)`, UTF-8 encoded. PyYAML's `safe_dump` is deterministic under these flags (sorted keys, block style only, no flow style fallback). The hash field is REMOVED before serialization (not set to null/empty-string) so a payload with the field absent and a payload with the field set hash identically. Cross-implementation portability requires the verifier to use the same PyYAML version family (>= 6.x); v2.1+ may pin an explicit YAML 1.2 canonicalizer if cross-implementation verification becomes a requirement.
...
- `iter_blob_sha` is the git blob SHA of the exact doc bytes, **written to the object DB at iter-1 dispatch via `git hash-object -w <doc-path>`**. The `-w` flag is essential — without it the SHA is a hash-only computation and `git cat-file -p` fails because no object exists. With `-w`, a loose blob object lands in `.git/objects/` and persists until `git gc` prunes it (default `gc.pruneExpire = 2.weeks.ago`; tunable via `git config gc.pruneExpire`). Iter-2 delta-review that runs after the configured prune window with an intervening `git gc` (auto-triggered routinely by commit/fetch/pack-objects under `gc.auto`) will fail-closed on retrieval — recovery is to re-dispatch a fresh full-doc iter-2 (via `--override-cap`, degraded mode, no delta-review provenance) per §Edge case E5.
...
| E21 | Sub-judge prompt-level tool restriction violated (sub-judge calls a tool not allowed by its rubric — e.g., `structure` sub-judge invokes Bash) | **Text-only output quarantine**, not tool-invocation-log inspection. Claude Code's Task tool returns the subagent's final textual response; per-call tool-trace metadata is not exposed to the orchestrator and `cli.spec_review` cannot observe which tools a sub-judge actually called. The defense layer that DOES exist is `cli/failure_attestation.py::quarantine_output` which regex-scans the returned text for (a) suspicious filesystem-secret paths (`.env`, `.ssh/`, `.aws/credentials`, `id_rsa`, ...), (b) credential-shaped strings (AWS / Stripe / GitHub / GitLab / JWT / OAuth / DB-URI / PEM markers), (c) prompt-injected location field shapes (`/etc/...`, `..` escapes, HTML tags, javascript: URLs — see `cli/aggregator.py::sanitize_location`). Hits → finding replaced with REDACTED marker, sub-judge `status: error`, `error_message: "tool_scope_violation"`. For optional sub-judge → E1 semantics. For mandatory sub-judge → E1b semantics (hard fail). NOTE: detection is after-the-fact and prose-only; a prompt-injected sub-judge that paraphrases / re-encodes / splits content across findings can evade. See §Security S1 for the honest trust model — orchestra is the wrong tool for sub-judges running on attacker-influenced inputs. |
...
| # | Concern | Mitigation |
|---|---|---|
| S1 | `repo-context` + `architectural-fit` sub-judges with Read+Grep+Glob tool access — broader than v1's read-only judge | **Trust model (honest, no false guarantees):** orchestra runs in the **user's own Claude session** on a **user-owned repo** as a **user-intentionally-installed plugin**. Sub-judges read scoped roots (`docs/`, `cli/`, `skills/`, `tests/`, `eval/`) to do their job — citation validation, impl-doc match, ADR consistency. v1 spec-review already reads the target doc; v2 broad-read sub-judges read additional in-repo files of the same kind. This is the same trust boundary as `pyrefly check`, `cli.lint`, or any other dev tool the user runs in this repo. **Defense-in-depth layers** (all present, all prose-only, none of them runtime sandboxes): (1) **Prompt-level path-scoping** — broad-read sub-judge prompts declare "you may only Read/Grep within `docs/`, `cli/`, `skills/`, `tests/`, `eval/`. Refuse Read on any other path. Refuse Bash. Refuse Write. Refuse Network." Advisory; a malicious or prompt-injected sub-judge can ignore it. (2) **Text-only output quarantine** (post-iter-2 honest-rewrite) — Claude Code's Task tool returns the subagent's final textual response only, not a per-call tool-invocation log; there is no surface where the orchestrator can observe which tools the sub-judge actually called. The defense layer that DOES exist scans the RETURNED TEXT via `cli/failure_attestation.py::quarantine_output` for credential-shaped strings (AWS / Stripe / GitHub / GitLab / JWT / OAuth / DB-URI / PEM markers — 25+ patterns) and via `cli/aggregator.py::sanitize_location` for prompt-injected location fields (absolute system paths, `..` escapes, HTML tags, javascript: URLs). Hits → finding redacted, sub-judge `status: error`. Detection is after-the-fact and prose-only — a prompt-injected sub-judge that already read a file then paraphrases / encodes / splits content evades the regex layer. (3) **Path canonicalization for PDSA citation + Refs validation** — `_resolve_inside_repo` rejects absolute paths and `..`-escapes before any `Path.read_text()`. **What this gets right**: accidental over-reads and stumbled-into-sensitive-paths whose RAW content would land in a finding are caught at output-quarantine and never persisted. **What this does NOT protect against**: a prompt-injected sub-judge that reads a sensitive file AND encodes contents in a way that evades the regex set, OR that simply describes secrets in prose. Real exfiltration risk exists at the sub-judge runtime layer because there is no infrastructure sandbox. **Operator guidance for sensitive-repo deployments**: orchestra is the wrong tool for repos containing `.env` / credentials / customer data within the scoped roots. Either (a) don't install orchestra in such a repo, or (b) keep secrets outside `docs/`, `cli/`, `skills/`, `tests/`, `eval/` (idiomatic anyway), or (c) fork orchestra and patch out `repo-context` + `architectural-fit` dispatch lines (~2 lines per sub-judge in `skills/spec-review/SKILL.md`). **What is NOT supported in v2**: untrusted-sub-judge threat models, multi-tenant / shared-infra / CI deployments where sub-judges run on attacker-influenced inputs. When orchestra is later deployed in CI / shared-infra contexts, a future LLD will add hard sandbox + tool-allowlist primitives. |
...
Iter-1 attestation (`docs/reviews/011-spec-review-v2-r1.review.yaml`) carried 13 class-scoped findings. Per LLD-011 §Design class-vs-instance fix protocol, iter-2 records how each class finding was swept across the body.

| Class finding (iter-1 location) | Sweep outcome | Audit count |
|---|---|---|
| Design § Delta-review on iter 2 — `git hash-object -w` side-effect | Code fix #1: widen except clause + recovery guidance in `_run_aggregate_and_write` (cli/spec_review.py). LLD body §Design Provenance paragraph updated with `gc.pruneExpire` reality + recovery path. No other body location referenced the side-effect. | 2 (code + doc) |
| Design § PDSA item 3 — citation reads arbitrary paths | Code fix #2: new `_resolve_inside_repo` helper in cli/pdsa.py applied to both `_check_citations` AND `_check_refs`. All file:line + Refs: validation paths re-grep'd; no other PDSA check reads cited paths. | 2 (citations + refs) |
| Design § Schema v2.0 — integrity hash canonicalization underspecified | LLD §Design Schema v2.0 paragraph fully specifies algorithm (`safe_dump(sort_keys=True, default_flow_style=False)`, hash field removed not zeroed, PyYAML 6.x family pin). `cli/spec_review.py::_compute_attestation_integrity_hash` implementation already matches the spec — verified line-by-line. | 2 (doc + code-grep verification) |
| Security § S1 — credential regex narrow | Code fix #3: `_SUSPICIOUS_PATH_PATTERNS` in cli/failure_attestation.py expanded 7 → 25+. Single source-of-truth; no parallel regex set elsewhere in codebase (re-grep'd `cli/`, `skills/`, `tests/`). | 1 |
| Design § Aggregator — fuzzy_hash collision suppression | LLD §Edge cases E14 clarified: max-severity wins for merged severity, raised_by union preserves attributions, canonical text from alphabetically-first id is documented lossy choice. No code change (trade-off accepted per LLD). Sweep: `raised_by` length > 1 in iter-2 attestation flags collisions as a class signal. | 1 (doc only) |
| Design § Aggregator + Schema — location not path-shape-rejected | Code fix #4: new `aggregator.sanitize_location` applied in `aggregate_findings` before dedup. Single integration point; no other code path inserts finding locations into attestation or chat report. | 1 |
| Design § PDSA item 3 — citation regex misses novel forms | Trade-off accepted as iter-2 backlog (not a Critical). PDSA still only matches `<path>:<N>` and `<path>:<N>-<M>` forms; novel cite shapes (file:50,55 / file:50ff / file:#anchor) silently pass validation. Class scope: applies to any doc with non-canonical cite syntax. Mitigation tracked as v2.1 backlog. | 0 (deferred, documented as class-known) |
| Design § Schema + Mandatory-map versioning | LLD §Design Mandatory map paragraph documents `MANDATORY_SUBJUDGES` as schema-version-bound. Iter-2 attestation under a binary with a different mandatory set would fail-validate against schema-2.0 (sub_judge.id enum + mandatory flag mismatch). Cross-version sweep deferred to v2.1+. | 1 (doc) |
| Security § S1 — prompt-level path scoping | Trade-off accepted per LLD §Security S1 honest-trust-model paragraph. Mitigations already documented (output-quarantine, "wrong tool for repos with secrets" operator guidance). No additional sweep needed in iter-2; remains class-scope risk for sensitive-repo operators. | 0 (already documented) |
| Edge Cases § E18 + doc disappeared | Already covered by Phase 3 slice 3.18 (failure-attestation invariant). Test coverage in tests/test_failure_attestation.py::test_e18_doc_disappeared_attestation_built. | 1 (verified) |
| Design § PDSA item 7 — bare-name design supersession | BUG-014 gap acknowledged; PDSA filename_grammar regex for `design` doc type already permits `<name>-r<N>.md`. Sweep confirmed all 6 doc-type patterns. | 1 |
| Design § Override-cap iter-3 — Critical-bypass concern | LLD §Design 2-iter cap interview-gate options paragraph clarified: "No, accept iter-2 as final" option is an EXPLICIT operator choice with audit (recorded in attestation notes via `--override-cap`). Class-scope risk surfaces at operator-decision time, not silently. | 1 (doc) |
| Design § Aggregator canonical-text ordering bias | LLD §Edge cases E14 explicitly documents the alphabetically-first-id text-pick as the trade-off. raised_by length > 1 surfaces collisions. Bias is bounded (one sub-judge's framing per merged entry); does not affect severity ranking. | 1 (doc) |

**Aggregate sweep status**: 13/13 class findings audited. 9 swept via code or doc edits; 4 accepted as trade-offs with documented mitigation path (v2.1 backlog or operator-decision boundary).
...
| 2026-05-11 | Status flip Draft → Approved per Hassan approval. Doc is now canon-frozen — future body changes require narrow-change rule (Addresses: lines per finding) or supersession. Implementation may proceed: write `docs/plans/2026-05-11-spec-review-v2-implementation.md`; user approves plan; execute Phase 1 + 2 + 3 via TDD vertical slicing; self-apply v2 reviewer on LLD-011 once shipped (dogfood); measure depth metric on 5-doc sample vs codex; tag v2.0.0. |
...
| 2026-05-12 | Iteration 1 → 2 narrow-change addressing orchestra v2 first dogfood attestation (`docs/reviews/011-spec-review-v2-r1.review.yaml` — 9 Critical findings). Addresses: docs/reviews/011-spec-review-v2-r1.review.yaml gate adversarial finding 1 (Critical) — `git hash-object -w` failure path now widens to `FileNotFoundError + PermissionError + CalledProcessError` and surfaces LLD-prescribed recovery guidance in error message (see commit "fix: address LLD-011 iter-1 Critical adversarial findings"); no API surface change. |
| 2026-05-12 | Addresses: docs/reviews/011-spec-review-v2-r1.review.yaml gate adversarial finding 2 (Critical) — git gc.pruneExpire default corrected in §Edge cases E5: was "≥14 days, typically months" → actual git default is `2 weeks`, tunable via `git config gc.pruneExpire`. Recovery via `--override-cap` documented as expected post-prune behavior. |
| 2026-05-12 | Addresses: docs/reviews/011-spec-review-v2-r1.review.yaml gate adversarial finding 3 (Critical) — PDSA citation + Refs: checks now canonicalize via new `_resolve_inside_repo` helper. Absolute paths (`/etc/passwd:1`) and `..`-escapes (`../../../etc/shadow`) are rejected before any `Path.read_text()`; defends against arbitrary file read via crafted cite. |
| 2026-05-12 | Addresses: docs/reviews/011-spec-review-v2-r1.review.yaml gate adversarial finding 4 (Critical) — `attestation_integrity_hash` canonicalization fully specified in §Design Schema v2.0: `yaml.safe_dump(payload_with_hash_field_omitted, sort_keys=True, default_flow_style=False)` then SHA-256 of UTF-8 bytes. PyYAML safe_dump is deterministic under these flags; documented as the algorithm-of-record. |
| 2026-05-12 | Addresses: docs/reviews/011-spec-review-v2-r1.review.yaml gate adversarial finding 5 (Critical) — `_SUSPICIOUS_PATH_PATTERNS` in `cli/failure_attestation.py` expanded from 7 patterns to 25+ (Stripe sk/pk_live, GitHub ghp/gho/ghu/ghr, GitLab glpat, JWT, OpenAI/Anthropic sk-/sk-ant, OAuth Bearer + refresh, DB URIs with creds, PEM markers, .netrc, .pgpass). §Security S1 paragraph 2 references the expanded set; +6 regression tests in `test_failure_attestation.py`. |
| 2026-05-12 | Addresses: docs/reviews/011-spec-review-v2-r1.review.yaml gate adversarial finding 6 (Critical) — fuzzy_hash collision suppression risk clarified in §Edge cases E14: when two distinct findings collide on `(normalize_location, fuzzy_hash)` AND severities differ, max-severity wins for the merged severity but raised_by union preserves both sub-judge attributions, and the canonical text (alphabetically-first id) is documented as the lossy choice. Trade-off accepted per LLD design; mitigation path is to surface the loss via §Scope item 14 audit on iter-2 (raised_by length > 1 is a signal that distinct findings merged). |
| 2026-05-12 | Addresses: docs/reviews/011-spec-review-v2-r1.review.yaml gate adversarial finding 7 (Critical) — new `aggregator.sanitize_location` rejects locations containing absolute system paths, `..`-escapes, HTML/XML tags, javascript:/data: URLs, control chars, or implausibly long strings. Applied in `aggregate_findings` before dedup so prompt-injected sub-judges can't smuggle path-shapes or markup into persisted attestation / chat report. +6 regression tests in `test_aggregator.py`. |
| 2026-05-12 | Addresses: docs/reviews/011-spec-review-v2-r1.review.yaml gate adversarial finding 8 (Critical) — Opus SPOF for mandatory sub-judges (semantic + adversarial both run on claude-opus-4-7) acknowledged in §Edge cases E17b + §Security threat-model: documented as an accepted single-vendor risk. Recovery path when Opus is unavailable: operator can downshift mandatory sub-judges to Sonnet 4.6 via a future config (post-v2.0.0 work — tracked separately, not blocking tag) OR fire `--override-cap` to ship an iter-N attestation marked degraded. |
| 2026-05-12 | Addresses: docs/reviews/011-spec-review-v2-r1.review.yaml gate repo-context finding 1 (Critical) — `test_tool_trace_rejection.py` reference in §Testing Strategy + iter-1 Changelog corrected: E21 quarantine tests ship in `tests/test_failure_attestation.py` (slices 3.20-3.22 — `test_e21_quarantine_redacts_*` + `test_e21_*_quarantine_*_fail`). The dedicated `test_tool_trace_rejection.py` file was folded into the broader failure-attestation test module during Phase 3 execution; LLD body now matches the shipped file layout. |
| 2026-05-12 | Addresses: docs/reviews/011-spec-review-v2-r2.review.yaml gate adversarial finding 3 (Critical) — E10 no-op iteration documented bypass closed. `cli/delta_review.py::assert_diff_non_empty` raises `SpecReviewError(noop_iteration_refused)` when iter-N+1 dispatch finds the doc bytes unchanged from iter-N. Bumping `Iteration:` without editing the doc now fails-closed instead of silently re-using iter-N findings as iter-N+1's attestation. +2 regression tests in `test_delta_review.py`. |
| 2026-05-12 | Addresses: docs/reviews/011-spec-review-v2-r2.review.yaml gate adversarial finding 5 (Critical) — §E21 + §Security S1 rewritten to remove the unimplementable "tool-trace metadata inspection" claim. Claude Code's Task tool returns the sub-judge's final textual response only; per-call tool-invocation logs are not surfaced to the orchestrator. The defense layer that actually exists is text-only output-quarantine (`quarantine_output` + `sanitize_location` — credential regex set, location path-shape rejection, HTML/control-char stripping); the LLD now describes this honestly as prose-only detection that catches accidental over-reads but does not defend against a prompt-injected sub-judge that paraphrases. No code change — this is a doc-accuracy fix to remove a marketing-stronger-than-implementation claim. |
| 2026-05-12 | Status flip Approved → Implemented. All three phases (1 + 2 + 3) + self-application shipped. Plugin `plugin.json` + `marketplace.json` bumped to 2.0.0. Test suite 509 pass / 0 pyrefly errors / 12/12 eval scenarios. v2 code dogfooded on this LLD across iter-1 + iter-2 (43 → 27 findings; 9 Critical → 5 Critical with the 5 remainder being documented v2.1-backlog trade-offs in §Out of Scope). `docs/reviews/011-spec-review-v2-r1.orchestra.review.yaml` + `-r2.orchestra.review.yaml` are the dogfood attestations of record. Remaining steps before `git tag v2.0.0`: plan Status flip Approved → Implemented; final HANDOFF.md ship entry; user runs `git tag v2.0.0 && git push --tags` (release action — operator decision, not automatable). |

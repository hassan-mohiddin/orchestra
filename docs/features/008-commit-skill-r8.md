...
- [ ] **A3.** Hook templates relocated `cli/templates/{pre-commit.sh,commit-msg.sh}` → `skills/commit/templates/`. **r8 update:** `attestation-template.yaml` relocated `cli/templates/` → `skills/spec-review/templates/` per architectural cleanup (spec-review skill owns its own templates per LLD-008 placement rule). `cli/templates/` now retains **10 init-related artifacts** (was 11 in r7): mkdocs.yml, AGENTS.md.template, llms.txt.template, standards-default-7.md, docs-index.md, mkdocs_hooks.py, orchestra-lint.yml, requirements-docs.txt, tags.md, precommit-yaml-patch.txt. `precommit-yaml-patch.txt` stays — different purpose (BUG-007 yaml-checker `--unsafe`). → test T2 (presence assertions on cli/templates/ + skills/commit/templates/ + skills/spec-review/templates/).
